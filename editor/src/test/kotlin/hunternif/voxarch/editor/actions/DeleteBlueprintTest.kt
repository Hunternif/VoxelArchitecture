...
class DeleteBlueprintTest : BaseAppTest() {
    ...
    private lateinit var reg: SceneRegistry
    ...
    fun `remove blueprint from node, undo redo`() {
        ...
        assertEquals(listOf(node1, node2), reg.bpInNodes[bp1])
        assertEquals(listOf(node1), reg.bpInNodes[bp2])
        ...
        assertEquals(listOf(bp1, bp2), reg.blueprintIDs.map.values.toList())
        assertEquals(listOf(node2), reg.bpInNodes[bp1])
        assertEquals(listOf(node1), reg.bpInNodes[bp2])
        ...
        assertEquals(bp1, app.state.selectedBlueprint)
        assertEquals(listOf(bp1, bp2), reg.blueprintIDs.map.values.toList())
        assertEquals(setOf(node1, node2), reg.bpInNodes[bp1].toSet())
        assertEquals(listOf(node1), reg.bpInNodes[bp2])
        assertEquals(listOf(bp1, bp2), node1.blueprints)
        assertEquals(listOf(bp1), node2.blueprints)
    ...
    fun `delete blueprint from library, undo redo`() {
        ...
        assertEquals(emptyList<SceneNode>(), reg.bpInNodes[bp1])
        assertEquals(listOf(node1), reg.bpInNodes[bp2])
        assertEquals(listOf(bp2), node1.blueprints)
