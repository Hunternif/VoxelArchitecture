...
open class Array2D<T>(
    ...
    companion object {
        inline operator fun <reified T> invoke(
            ...
            height: Int,
            init: (x: Int, y: Int) -> T
        ) = invoke(length, height, init(0, 0)).apply {
            for (p in this) {
                set(p, init(p.x, p.y))
            }
        }
        ...
        inline operator fun <reified T> invoke(
            ...
            height: Int,
            init: T
        ) = Array2D(length, height, Array(length * height) { init })
    ...
    open operator fun contains(p: IntVec2) = p.x in 0 until length && p.y in 0 until height
